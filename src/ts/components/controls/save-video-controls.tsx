...
export const SaveVideoControls = (
  props: SaveVideoControlsProps
): JSX.Element => {
  const { propsOptions, propsPlayer, propsPlayUrl, propsDisplayClickPlay } =
    props;
  ...
  const setControlAreaFadeInOut = (): void => {
    ...
    const playerCtrl = document.querySelector(
      `#${propsOptions.parentElementId} .dms-player-save-video-contorl`
    ) as HTMLDivElement;
        ...
        setPlayTimeInterval(
          window.setInterval(() => {
            setCurrentDate((currentDate) => {
              if (
                currentDate &&
                propsOptions.endDate &&
                currentDate < propsOptions.endDate
              ) {
   ...
   * 저장 영상 재생
   */
  const saveVideoPlay = (playOptions?: {
    ...
    if (player) {
      ...
      const startDate = playOptions?.date
        ? playOptions.date
        : propsOptions.startDate;
    ...
    <div className="dms-player-save-video-contorl">
      ...
      <div className="control-box-container">
        <div className="left-controls control-box">
          ...
          {propsOptions.playSpeed ? (
            <button
              className={`control-btn ${
                playSpeedBtnDisabled ? "disabled" : ""
              }`}
              onClick={() => setIncreasePlaySpeed()}
              aria-label="저장 영상 배속 증가"
            >
          ...
          {propsOptions.playSpeed ? (
            <div className="play-speed">x{(playSpeed / 10).toFixed(1)}</div>
          ) : null}
        ...
        <div className="center-controls control-box">
          {utcToDateString(currentDate)} /{" "}
          {utcToDateString(propsOptions.endDate)}
        </div>
