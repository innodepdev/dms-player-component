...
export const LiveStreamControls = (
  props: LiveStreamControlsProps
): JSX.Element => {
  const {
    propsTogglePtzButtonControl,
    propsOptions,
    propsPlayer,
    propsPlayUrl,
  ...
  const setBtnTooltipPosition = (): void => {
    ...
    const tooltip = document.querySelector(
      `#${propsOptions.parentElementId} ${CONTROL_AREA_CLASS} .btn-tooltip`
    ) as HTMLDivElement;
    // btn elements
    const ctrlBtns = document.querySelectorAll(
      `#${propsOptions.parentElementId} ${CONTROL_AREA_CLASS} .control-btn`
    );
    ctrlBtns.forEach((btn: Node) => {
      (btn as HTMLButtonElement).addEventListener(
        ...
        (e: MouseEvent) => {
          ...
          const calcRight = Number(
            `${target.offsetLeft + target.clientWidth + target.clientWidth / 2}`
          );

          // 전체 warp 영역 element & width
          const playerWrap = document.querySelector(
            `#${propsOptions.parentElementId} .dms-player-wrap`
          ) as HTMLDivElement;
  ...
  const setControlAreaFadeInOut = (): void => {
    ...
    const playerCtrl = document.querySelector(
      `#${propsOptions.parentElementId} .dms-player-live-stream-contorl`
    ) as HTMLDivElement;
    ...
    playerWrap.addEventListener("mouseleave", () => {
      ...
      const ptzListWrap = document.querySelector(
        ".ptz-list-wrap"
      ) as HTMLDivElement;
      if (ptzListWrap) {
  ...
  const removePlayerElement = (): void => {
    const video = document.querySelector(
      `#${propsOptions.parentElementId} .dms-player-container video`
    );
    if (video) {
      video.remove();
    }
  ...
  const togglePtzList = (e: Event): void => {
    ...
    const ptzListWrap = document.querySelector(
      `#${propsOptions.parentElementId} .ptz-list-wrap`
    ) as HTMLDivElement;
  ...
  const removePtzCanvas = (): void => {
    const canvasEl = document.querySelector(
      `#${propsOptions.parentElementId} .dms-player-wrap canvas`
  ...
  return (
    <div className="dms-player-live-stream-contorl">
      {propsOptions.buttonTooltip ? <div className="btn-tooltip"></div> : null}
      {player ? (
          ...
          <div className="left-controls control-box">
            {isPlay ? (
              <button
                ...
                onClick={() => liveStreamPause()}
                aria-label={PLAY_STOP_TITLE}
              >
            ...
            {propsOptions.capture ? (
              <button
                ...
                onClick={() => getVideoCaptureImage(player)}
                aria-label="실시간 영상 캡쳐"
              >
            ...
            {propsOptions.ptz ? (
              <div className="ptz-list-wrap">
                <ul>
                  <li
                    onClick={(e) => toggleButtonPtzControl(e)}
                    aria-label="버튼 PTZ"
                  >
                    <span>버튼 PTZ</span>
                  </li>
                  <li
                    onClick={(e) => toggleDragPtzControl(e)}
                    aria-label="드래그 PTZ"
                  >
                    <span>드래그 PTZ</span>
                  </li>
                  <li
                    onClick={(e) => toggleAreaPtzControl(e)}
                    aria-label="영역 줌"
                  >
                    <span>영역 줌</span>
                  </li>
                  <li
                    onClick={(e) => togglePoinPtzControl(e)}
                    aria-label="포인트 이동"
                  >
                    <span>포인트 이동</span>
                  </li>
